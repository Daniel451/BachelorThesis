...
# network stuff
from cnn_models.models.abstr_model_17 import abstr_model_s11 as abstr_net
...
class NetworkModel(abstr_net):
    ...
    def __build_model(self):
        ...
        # conv/maxpool layer
        self.W_conv1 = weight_variable([9, 9, 3, 16], "W_conv1")
        self.b_conv1 = bias_variable([16], "b_conv1")

        h_conv1 = activation(conv2d(x_image, self.W_conv1) + self.b_conv1)
        h_pool1 = max_pool_2x2(h_conv1)
        h_pool1 = tf.nn.dropout(h_pool1, self.keep_prob)

        # conv/maxpool layer
        self.W_conv2 = weight_variable([5, 5, 16, 32], "W_conv2")
        self.b_conv2 = bias_variable([32], "b_conv2")

        h_conv2 = activation(conv2d(h_pool1, self.W_conv2) + self.b_conv2)
        h_pool2 = avg_pool_2x2(h_conv2)
        h_pool2 = tf.nn.dropout(h_pool2, self.keep_prob)

        # conv/maxpool layer
        self.W_conv3 = weight_variable([3, 3, 32, 32], "W_conv3")
        self.b_conv3 = bias_variable([32], "b_conv3")

        h_conv3 = activation(conv2d(h_pool2, self.W_conv3) + self.b_conv3)
        h_pool3 = h_conv3
